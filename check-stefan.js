...
async function checkStefanData() {
  try {
    ...
    if (!user) {
      console.log('User stefan@xodo.ro not found');
      return;
    }
    ...
    console.log(`\nTotal accumulated: ${totalSeconds}s = ${(totalSeconds / 3600).toFixed(2)}h`);
    console.log(`Dashboard format: ${Math.floor(totalSeconds / 3600)}:${Math.floor((totalSeconds % 3600) / 60).toString().padStart(2, '0')}`);

    // Find active subscription
    const activeSub = allSubs.find(s => s.status === 'ACTIVE');
    if (activeSub) {
      console.log(`\n📊 Current active subscription: ${activeSub.plan?.name} (${activeSub.hoursUsed}h used)`);
    }
