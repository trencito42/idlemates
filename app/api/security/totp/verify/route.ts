...
import { envelopeDecryptDataKey, decryptWithDataKey, encryptWithDataKey } from '@/lib/crypto'
...
export async function POST(req: Request) {
  ...
  await prisma.user.update({ where: { id: user.id }, data: { last2FAAt: new Date() } as any })
  // Mark recent 2FA verification (valid for 5 minutes)
  try {
    const redis = getRedis()
    await redis.set(`2fa:recent:${user.id}`, '1', 'EX', 300)
  } catch {}
